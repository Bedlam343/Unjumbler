...
Dictionary::Dictionary(string fileName) {
    ...
    if(!infile) {
        cout << "File named \"" << fileName << "\" not found!" << endl;
        cout << "Exiting program...hit any key.";
        getchar();
        exit(0);
    }
...
Dictionary::Dictionary(const Dictionary &d) {
    for(int outer = 0; outer < tableSize; outer++) {
        ...
        while(otherCurr != nullptr) {
            ...
            vector<string>words = otherCurr->getWords();
            for(unsigned int inner = 0; inner < words.size(); inner++) {
                thisNew->insert(words[inner]);
            }
            insertNew(outer, thisNew);
            otherCurr = otherCurr->next;
        }
    }
    numWords = d.getNumWords();
}
...
bool Dictionary::removeWord(string w) {
    const LetterBag currKey(w);
    int index = currKey.hashValue(tableSize);
    AnagramSet *currAnagram = hashTable[index];
    AnagramSet *prevAnagram = nullptr;

    while(currAnagram != nullptr) {
        if(currAnagram->key == currKey) {
            if(currAnagram->contains(w)) {
                ...
                if(currAnagram->getSize() == 0) {
                    //If previous == nullptr, that means the one being deleted is at the hasTable[index]
                    if(prevAnagram == nullptr) {
                        hashTable[index] = currAnagram->next;
                        delete currAnagram;
                    }
                    //Current Anagram is the last one in the linkedList;
                    else if (currAnagram->next == nullptr) {
                        prevAnagram->next = nullptr;
                        delete currAnagram;
                    }
                    //Somewhere in the middle of a linked list
                    else {
                        prevAnagram->next = currAnagram->next;
                        delete currAnagram;
                    }
                }
                //Word has been removed, return true;
                return true;
            }
            //If the word wasn't in the matching Anagram, break out of the while loop
            return false;
        }
...
bool Dictionary::contains(const string &word) const {
    ...
    return (match == nullptr)? false : match->contains(word);
}

vector<string> Dictionary::getWords(const string & word) const {
    AnagramSet *match = findWord(word);
    return (match == nullptr)? vector<string>(0) : match->getWords();
}

int Dictionary::getNumAnagrams(const string & word) const {
    AnagramSet *match = findWord(word);
    return (match == nullptr)? 0 : match->getSize();
}

void Dictionary::printDictionary(int limit, ostream & out) const {
    out << endl;
    for(int counter = 0; counter < tableSize && limit > 0; counter++) {
        AnagramSet *currAnagram = hashTable[counter];
        if(currAnagram != nullptr) {
            out << "\t" << counter << "..." << "     ";
            while(currAnagram != nullptr) {
                out << *currAnagram << "  ";
    ...
    out << "Number of words: " << numWords << "   " << "tableSize = " << tableSize << endl << endl;
}
